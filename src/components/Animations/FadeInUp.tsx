...
const FadeInUp = ({ latestWorks }: FadeInUpProps) => {
  ...
  return (
    <div
      ref={containerRef}
      className="mt-10 flex h-[2500px] flex-col items-center justify-center space-y-6 lg:grid lg:h-[1200px] lg:grid-cols-2 lg:gap-6 lg:space-y-0"
    >
      {isInView && (
        <>
          {visibleItems.map((latest, i) => {
            return (
              <div
                key={i}
                className="animate-fadeInUp hadow-lg w-full rounded-lg border border-black/20 bg-white px-5 pb-6 pt-5 dark:border-white/50 dark:bg-[#18181b] dark:shadow-[#18181b]"
              >
                      ...
                      {latest.learnMore && (
                        <Dialog>
                          <DialogTrigger asChild>
                            <Button
                              className="flex p-2 text-sm sm:text-base"
                              variant="link"
                            >
                              Important info &rarr;
                            </Button>
